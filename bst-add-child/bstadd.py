...
class Node(object):
    ...
    def insert(self, new_data):
        ...
        # check to see if the new node data is higher or lower than current node
        if new_data < self.data:
            # if data is lower, check to see if there is a left child node
            if self.left:
                # if so, recurse on that node
                self.left.insert(new_data)
            else:
                # if not, add a new node to the left with the new data
                self.left = Node(new_data)
        else:
            # if data is higher, same process on the right
            if self.right:
